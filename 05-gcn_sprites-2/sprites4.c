...
void update_sprite(Sprite* oneSprite, Positions* oldPositions, int currentTime)
{
    ...
    if (oneSprite->destX < 0)
    {
        ...
        /* Recalage sur le bord */
        oneSprite->destX = 0;

        /* Nouvelle vitesse vers la droite */
        oneSprite->dispX = get_random(1, 10);
    }
    if (oneSprite->destX > 639 - 32)
    {
        /* Touche le bord droit */
        /* 639 est la dernière valeur d'une fenêtre de 640 de large */
        /* 32 est la largeur du sprite */
        /* Recalage sur le bord */
        oneSprite->destX = 639 - 32;

        /* Nouvelle vitesse vers la gauche */
        oneSprite->dispX = get_random(-10, -1);
    }
    if (oneSprite->destY < 0)
    {
        /* Nouvelle vitesse vers le bas */
        oneSprite->dispY = get_random(1, 10);
    }
    if (oneSprite->destY > 479 - 32)
    {
        /* Touche le bord bas */
        /* 479 est la dernière valeur d'une fenêtre de 480 de large */
        /* 32 est la largeur du sprite */
        /* Recalage sur le bord */
        oneSprite->destY = 479 - 32;

        /* Nouvelle vitesse vers le haut */
